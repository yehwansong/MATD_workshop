            ...
            function transform(){
                ...
                for (var i = elem_pos_array.length - 1; i >= 0; i--) {
                    ...
                    if(value < 0){value = 0}
                        ...
                        rotationx = map_range(value,0,0.5,min_rotationx,max_rotationx)

                        wght = map_range(value,0,0.5,min_wght,max_wght)
                        blur = map_range(value,0,0.5,10,0)
                        srff = map_range(value, 0,0.5,min_srff,max_srff)
                        // spacing = map_range(value,0,0.5,0,100)
                        ...
                        $('.wrapper').eq(localcounter).find('.element').eq(i).css({'filter':'blur('+blur+'px)'})
                        // $('.wrapper').eq(localcounter).find('.element').eq(i).css({'margin':'0 '+spacing+'px'})
                        $('.wrapper').eq(localcounter).find('.element').eq(i).css({'font-variation-settings':' "wght" '+wght+', "SRFF" '+srff+', "ital" 14'})
