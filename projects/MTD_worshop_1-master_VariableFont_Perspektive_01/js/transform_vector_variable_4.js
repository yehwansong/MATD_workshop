    ...
    transform()
            ...
            function transform(){
                if((pre_value_x!==value_x)||(pre_value_y!==value_y) && value_x>0 && value_y>0){
                            ...
                            var min_translate_x = -1100
                            var max_translate_x = -1100
                ...
                for (var i = elem_pos_array.length - 1; i >= 0; i--) {
                    ...
                    if(value < 0){value = 0}
                        console.log(value_x)
                        translate_x = map_range(value_x,0,1,min_translate_x,max_translate_x)
                        translate_y = map_range(value_x,0,1,min_translate_y,max_translate_y)
                        skew_x = map_range(value_x,0,1,min_skew_x,max_skew_x)
                        skew_y = map_range(value_x,0,1,min_skew_y,max_skew_y)
                        rotation = map_range(value_x,0,1,min_rotation,max_rotation)
                        scale_x = map_range(value_x,0,1,min_scale_x,max_scale_x)
                        scale_y = map_range(value_x,0,1,min_scale_y,max_scale_y)
                        ...
                        $('.wrapper').eq(localcounter).find('.element').eq(i).css({'font-variation-settings':' "wght" '+wght})
                }
                pre_value_x=value_x
                pre_value_y=value_y
