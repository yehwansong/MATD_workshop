            ...
            function transform(){
                ...
                for (var i = elem_pos_array.length - 1; i >= 0; i--) {
                    ...
                    if(value < 0){value = 0}
                        ...
                        opacity = map_range(value,0,0.5,min_opacity,max_opacity)
                        color = map_range(value,0,0.5,min_color,max_color)

                        wght = map_range(value,0,0.5,min_wght,max_wght)
                        // wdth = map_range(value,0,0.5,min_wdth,max_wdth)
                        ...
                        $('.wrapper').eq(localcounter).find('.element').eq(i).css({'opacity':opacity})
                        $('.wrapper').eq(localcounter).find('.element').eq(i).css({'color':'rgb('+color+',255,255)'})
                        // $('.wrapper').eq(localcounter).find('.element').eq(i).css({'font-variation-settings':' "wght" '+wght+', "wdth" '+wdth})
                }
                pre_value_x=value_x
                pre_value_y=value_y
