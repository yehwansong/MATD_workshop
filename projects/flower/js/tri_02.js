...
// POSITION
    ...
    // $('.img_'+i).css({bottom:'20%',left:'20%',margin:'-'+($('img_').height() / 2)+'px 0 0 -'+($('img_').width() / 2)+'px'});
...
// GENERATOR
    ...
    $('.img_'+i).css({'transform':'rotate('+(20/5)*i+'deg) '})///
    $('.img_'+i).css({'transform':'rotateY('+(20/5)*i+'deg)'})
    $('.img_'+i).css({'transform':'rotateZ('+(i*7-70)+'deg)'})

    $('.img_'+i).css({'transform':'rotateZ('+(i*7-70)+'deg) translateY('+i/400+'px)'})
    // $('.img_'+i).css({'transform', (i*1-1)+') translate('+((i*0)-200)+'px)'}) // Flip along horizontal axis
