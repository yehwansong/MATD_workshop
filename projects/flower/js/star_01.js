...
// ORIGIN
    ...
    $('.img_'+i).css({'transform-origin':'center right'})

// POSITION

    // $('.img_'+i).css({bottom:'35%',left:'43%',margin:'-'+($('img_').height() / 2)+'px 0 0 -'+($('img_').width() / 2)+'px'});

// SIZE

    $('.img_'+i).width(200); // Units are assumed to be pixels
    $('.img_'+i).height(200);


// GENERATOR

    // $('.img_'+i).css({'transform':'rotate('+(360/30)*i+'deg) scale('+(i%2+5)/5+') translateY('+i*5+'px)'})


    $('.img_'+i).css({'transform':'rotate('+(12/2)*i+'deg) scale('+((i%5)/2+0.2)+') '})///
    $('.img_'+i).css({'transform':'rotateY('+(12/2)*i+'deg)'})
    $('.img_'+i).css({'transform':'rotateZ('+(i*12-120)+'deg)'})


    $('.img_'+i).css({'transform':'rotateZ('+(i*20-12)+'deg) translateY('+i*2+'px)'})
    $('.img_'+i).css({'z-index':i%12})
