...
// POSITION
    ...
    // $('.img_'+i).css({bottom:'15%',left:'30%',margin:'-'+($('img_').height() / 2)+'px 0 0 -'+($('img_').width() / 2)+'px'});
...
// GENERATOR
    ...
    $('.img_'+i).css({'transform':'rotate('+(360/60)*i+'deg) scale('+((i%5)/5+0.1)+') translateX('+i*5+'px)'})///
    // $('.img_'+i).css({'transform':'rotateY('+(30/3)*i+'deg)'})
    // $('.img_'+i).css({'transform':'rotateZ('+(i*2-30)+'deg)'})


    // $('.img_'+i).css({'transform':'rotateZ('+(i*2-30)+'deg)'})
    // $('.img_'+i).css({'z-index':i%25})
