...
// POSITION
    ...
    // $('.img_'+i).css({top:'1%', bottom:'0',left:'25%',margin:'-'+($('img_').height() / 2)+'px 0 0 -'+($('img_').width() / 2)+'px'});
...
// GENERATOR
    ...
    $('.img_'+i).css({'transform':'rotate('+(360/10)*i+'deg) scale('+(i%2+3)/4+')'}) //scale('+(i%2+0.5)/3+') 
 ...
 // *****
            ...
            if(i == 0){
                ...
                if((window.innerHeight-$('body').outerHeight())<(max_bottom - min_top)){
                    $('.wrapper').css({'transform':'translate(-50%, -52%) scale('+(window.innerHeight-$('body').outerHeight())/(max_bottom - min_top)+')'})
                }
