...
// POSITION
    ...
    // $('.img_'+i).css({bottom:'0'});
    // $('.img_'+i).css({left:'25%'});
    // $('.img_'+i).css({margin:(-1*$('.img_'+i).outerHeight() / 2)+'px 0 0 -'+($('.img_'+i).outerWidth() / 2)+'px'});
...
// GENERATOR
    ...
    $('.img_'+i).css({'transform':'rotate('+(360/5)*i+'deg) '}) //scale('+(i%2+0.5)/3+') 
...
// *****
            ...
            if(i == 0){
                ...
                if((window.innerHeight-$('body').outerHeight())<(max_bottom - min_top)){
                    $('.wrapper').css({'transform':'translate(-50%, -50%) scale('+(window.innerHeight-$('body').outerHeight())/(max_bottom - min_top)+')'})
                }
