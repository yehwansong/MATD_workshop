            ...
            function transform(){
                ...
                for (var i = elem_pos_array.length - 1; i >= 0; i--) {
                    ...
                    if(value < 0){value = 0}
                        ...
                        $('.wrapper').eq(localcounter).find('.element').eq(i).css({'transform':'translate('+translate_x+'vh, '+translate_y+'px) skew('+skew_x+'deg, '+skew_y+'deg) rotate('+rotation+'deg) scale('+scale_x+', '+scale_y+')'})
                }
                pre_value_x=value_x
                pre_value_y=value_y
